...
def test_worker_openai_and_analysis_adapters(monkeypatch):
    assert worker.groq_headers()["Authorization"] == "Bearer test-key"
    ...
    custom_analysis = {**analysis, "custom": {"kyc_verified": True}}
    monkeypatch.setattr(worker.httpx, "post", lambda *args, **kwargs: Response({"choices": [{"message": {"content": json.dumps(custom_analysis)}}]}))
    result = worker.analyze("Transcript", [{"name": "kyc_verified", "type": "boolean"}])
    assert result["custom"] == {"kyc_verified": True}
...
def test_worker_processes_vapi_call(monkeypatch):
    call = {"call_sid": "CA1", "tenant_id": "tenant-a", "recording_url": "https://recording", "status": "queued", "provider": "vapi"}
    updates = []
